...
final class CameraStream extends Object
{
    ...
    CameraStream(final int cameraIndex, final boolean useFlashLight, final int port,
    ...
    void start()
    {
        synchronized (mLock)
        {
            if (mRunning)
            {
                throw new IllegalStateException("CameraStream is already running");
            } // if
            mRunning = true;
        } // synchronized
    ...
    /* package */ void stop()
    {
        synchronized (mLock)
        {
            if (!mRunning)
            {
                throw new IllegalStateException("CameraStream is already stopped");
            } // if
